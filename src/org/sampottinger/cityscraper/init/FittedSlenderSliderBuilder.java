...
{
	private static final SliderDirection DEFAULT_DIRECTION = SliderDirection.VERTICAL;
	...
	/**
	 * Create a builder for a slender slider with default center line showing, vertical direction, and default depth
	 * as defined by the Drawable interface
	 ...
	 * @param newTravel The new travel distance (in pixels) that the slider spans
	 * @param newAmountActive The source's range that is visible or otherwise "active at any given moment.
	 *                        This could be the number of pixels a scrollable area shows to the user at any given moment
	 */
	public FittedSlenderSliderBuilder(SliderSource newTargetSource, int newX, int newY, int newTravel, int newAmountActive)
	{
	...
	/**
	 * Sets the direction that this builder should use for the slender sliders it will create
	 * @param newDirection The new direction this builder should use during the
	 *                     construction of sliders
	 */
	public void setDirection(SliderDirection newDirection)
	{
	 ...
	 * @return New slider that fits the specifications previously given to the builder
	 */
	public SlenderSlider createSlider()
	{
		...
		// Calculate box size
		proportionalBoxSize = amountActive / (targetSource.getMaxValue() - targetSource.getMinValue()) * travel;
		actualBoxSize = proportionalBoxSize < MINIMUM_BOX_SIZE ? MINIMUM_BOX_SIZE : (int)proportionalBoxSize;
		
		// Create slider
		returnValue = new SlenderSlider(targetSource, x, y, actualBoxSize, travel, direction, showCenterLine, depth);
