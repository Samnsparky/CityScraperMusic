...
public class ScrollingCollectionFactory
{
	...
	private static ScrollingCollectionFactory instance;
	 ...
	 * @param boundsStrategyID The id of the strategy to use for the bounds
	 * 		calculation.
	 * @param movementStrategyID The id of the strategy to use for moving
	 * 		entities.
	 * @param targetEntities The entities to operate on.
	 * @param direction The direction in which the entities will travel.
	 * @param amountActive The number of pixels of this collection that will be
	 * 		visible at any given moment in time.
	 * @return New scrolling collection that fits the given specifications.
	 */
	public <T extends PhineasPlaceable & PhineasBoundable>
			ScrollingCollection<T> createCollection(
			ScrollingCollectionBoundsStrategyID boundsStrategyID,
			ScrollingCollectionMovementStrategyID movementStrategyID, 
			Iterable<T> targetEntities, Axis direction, int amountActive)
	{
	 ...
	 * @param incrementValue The minimum step the resulting scrolling collection
	 * 		can take.
	 * @return New scrolling collection that fits the given specifications.
	 */
	...
	public <T extends PhineasPlaceable & PhineasBoundable>
			...
			ScrollingCollectionBoundsStrategyID boundsStrategyID,
			ScrollingCollectionMovementStrategyID movementStrategyID, 
			Iterable<T> targetEntities, Axis direction,
			int amountActive, int startingValue, int incrementValue)
	{
		...
		{
		case TIGHT:
			boundsStrategy = new TightScrollingCollectionBoundsStrategy(
					direction, 
					(Iterable<PhineasBoundable>) targetEntities,
					amountActive
			);
			break;
		default:
		...
		{
		case DIRECT:
			movementStrategy = new DirectScrollingCollectionMovementStrategy(
					direction);
			break;
		default:
			throw new RuntimeException(
					"Invalid movement strategy id provided."
			);
		}
		
		returnVal = new ScrollingCollection<T>(
				boundsStrategy,
				movementStrategy,
				targetEntities,
				startingValue,
